/*
 * drivers/cpufreq/cpufreq_zeroactive.c
 *
...
static DEFINE_PER_CPU(struct cpufreq_zeroactive_cpuinfo, cpuinfo);
...
static int cpufreq_governor_zeroactive(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_ZEROACTIVE
static
#endif
struct cpufreq_governor cpufreq_gov_zeroactive = {
	.name = "zeroactive",
	.governor = cpufreq_governor_zeroactive,
...
static void cpufreq_zeroactive_timer_resched(
	struct cpufreq_zeroactive_cpuinfo *pcpu)
{
 ...
 * function.
 */
static void cpufreq_zeroactive_timer_start(int cpu)
{
	struct cpufreq_zeroactive_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int choose_freq_grid(
   struct cpufreq_zeroactive_cpuinfo *pcpu, unsigned int loadadjfreq, int cpu_load)
{
...
static unsigned int choose_freq(
	struct cpufreq_zeroactive_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_zeroactive_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	...
	int i, max_load;
	unsigned int max_freq;
	struct cpufreq_zeroactive_cpuinfo *picpu;
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_zeroactive_timer_resched(pcpu);
...
static void cpufreq_zeroactive_idle_start(void)
{
	struct cpufreq_zeroactive_cpuinfo *pcpu =
...
static void cpufreq_zeroactive_idle_end(void)
{
	struct cpufreq_zeroactive_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_zeroactive_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_zeroactive_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_zeroactive_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
...
static void cpufreq_zeroactive_boost(void)
{
	int i;
	int anyboost = 0;
...
static int cpufreq_zeroactive_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_zeroactive_notifier,
};
...
static struct attribute *zeroactive_attributes[] = {
...
static struct attribute_group zeroactive_attr_group = {
	.attrs = zeroactive_attributes,
	.name = "zeroactive",
};

static int cpufreq_zeroactive_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_zeroactive_idle_start();
		break;
	case IDLE_END:
		cpufreq_zeroactive_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_zeroactive_idle_nb = {
	.notifier_call = cpufreq_zeroactive_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_zeroactive_timer_start(j);
		...
		rc = sysfs_create_group(cpufreq_global_kobject,
				&zeroactive_attr_group);
		if (rc) {
			mutex_unlock(&gov_lock);
			return rc;
		}

		idle_notifier_register(&cpufreq_zeroactive_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_zeroactive_idle_nb);
...
static void cpufreq_zeroactive_nop_timer(unsigned long data)
{
}

static int __init cpufreq_zeroactive_init(void)
{
	unsigned int i;
	struct cpufreq_zeroactive_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_zeroactive_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_zeroactive_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_zeroactive_speedchange_task, NULL,
			       "cfzeroactive");
	...
	return cpufreq_register_governor(&cpufreq_gov_zeroactive);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ZEROACTIVE
fs_initcall(cpufreq_zeroactive_init);
#else
module_init(cpufreq_zeroactive_init);
#endif

static void __exit cpufreq_zeroactive_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_zeroactive);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
}

module_exit(cpufreq_zeroactive_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_zeroactive' - A cpufreq governor for "
