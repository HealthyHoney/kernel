/*
 * drivers/cpufreq/cpufreq_interactivez.c
 *
...
struct cpufreq_interactivez_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_interactivez_cpuinfo, cpuinfo);
...
static void cpufreq_interactivez_timer_resched(unsigned long cpu,
					      bool slack_only)
{
	struct cpufreq_interactivez_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	u64 expires;
	unsigned long flags;
 ...
 * function.
 */
static void cpufreq_interactivez_timer_start(int cpu)
{
	struct cpufreq_interactivez_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int choose_freq(
	struct cpufreq_interactivez_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_interactivez_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	int cpu_load;
	struct cpufreq_interactivez_cpuinfo *pcpu =
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_interactivez_timer_resched(data, false);
...
static void cpufreq_interactivez_idle_end(void)
{
	struct cpufreq_interactivez_cpuinfo *pcpu =
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		cpufreq_interactivez_timer_resched(smp_processor_id(), false);
	} else if (time_after_eq(jiffies, pcpu->cpu_timer.expires)) {
		del_timer(&pcpu->cpu_timer);
		del_timer(&pcpu->cpu_slack_timer);
		cpufreq_interactivez_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_interactivez_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			struct cpufreq_interactivez_cpuinfo *pjcpu;
			u64 hvt = 0;
...
static void cpufreq_interactivez_boost(void)
{
	int i;
	int anyboost = 0;
...
static int cpufreq_interactivez_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_interactivez_notifier,
};
...
static struct attribute *interactivez_attributes[] = {
...
static struct attribute_group interactivez_attr_group = {
	.attrs = interactivez_attributes,
	.name = "interactivez",
};

static int cpufreq_interactivez_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	if (val == IDLE_END)
		cpufreq_interactivez_idle_end();

	return 0;
}

static struct notifier_block cpufreq_interactivez_idle_nb = {
	.notifier_call = cpufreq_interactivez_idle_notifier,
};

static int cpufreq_governor_interactivez(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			pcpu->last_evaluated_jiffy = get_jiffies_64();
			cpufreq_interactivez_timer_start(j);
		...
		rc = sysfs_create_group(cpufreq_global_kobject,
				&interactivez_attr_group);
		if (rc) {
			mutex_unlock(&gov_lock);
			return rc;
		}

		idle_notifier_register(&cpufreq_interactivez_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_interactivez_idle_nb);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (policy->min < pcpu->min_freq)
				cpufreq_interactivez_timer_resched(j, true);
			pcpu->min_freq = policy->min;
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVEZ
static
#endif
struct cpufreq_governor cpufreq_gov_interactivez = {
	.name = "interactivez",
	.governor = cpufreq_governor_interactivez,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_interactivez_nop_timer(unsigned long data)
{
}

static int __init cpufreq_interactivez_init(void)
{
	unsigned int i;
	struct cpufreq_interactivez_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_interactivez_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_interactivez_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_interactivez_speedchange_task, NULL,
			       "cfinteractivez");
	...
	return cpufreq_register_governor(&cpufreq_gov_interactivez);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVEZ
fs_initcall(cpufreq_interactivez_init);
#else
module_init(cpufreq_interactivez_init);
#endif

static void __exit cpufreq_interactivez_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_interactivez);
...
module_exit(cpufreq_interactivez_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_interactivez' - A cpufreq governor for "
